...
- **Encode plane**: handles image or video encoding and produces embeddings.
- **Language plane**: consumes the embeddings and performs text generation.
...
1. Both the language and encode instances receive requests:
   ...
   Encode instances receive the full multimodal payload
      ├─ Extract the text portion and run multimodal preprocessing
      └─ Enqueue into MultimodalEmbeddingBootstrapQueue

2. The language plane establishes a connection
...
3. The encode instance processes the request
   ...
   ├─ Run the vision encoder to produce ~8,000 embedding tokens
...
4. Encode → language data transfer
   ├─ Send the 8,000 tokens through Mooncake/RDMA
...
class MultimodalEmbeddingBootstrapQueue:
    """
    Coordinate connection setup and block allocation on the encode plane
...
class MooncakeEmbeddingSender:
    """
    Stream embedding payloads to the language plane
...
2. Execute the vision encoder on images or video frames.
...
class MultimodalLanguageRequest:
    """
    Request state tracker on the language plane
...
**Key properties**:
- **On-demand allocation**: determine the exact number of blocks from the token length; the language plane still honors its default allocation baseline.
- **Scatter/gather compatibility**: support non-contiguous block reads and writes.
- **Automatic reclamation**: free the blocks once the request completes.
...
- **Independent scaling**: encode and language planes can grow horizontally according to their respective workloads.
- **Adaptive memory footprint**: block allocations expand or shrink with real token counts while preserving defaults for cold starts.
- **Resilient streaming**: resume-transfer safeguards maintain progress when buffers saturate or transient faults occur.
- **Model-agnostic integration**: supports Qwen2.5-VL, Qwen3-VL, and other multimodal architectures with configurable tensor parallelism.
- **Operational visibility**: both planes expose cache usage and transfer states for capacity planning and incident response.
...
# Language plane: text generation
...
The current implementation disaggregates encode and language planes. The language plane can be extended to integrate with the prefill plane, enabling Encode/Prefill/Decode separation.
