...
Pass request-scoped image settings through `mm_sampling_kwargs`. DeepSeek-OCR
uses the official Gundam defaults when this field is omitted: a 1024-pixel
global view, 640-pixel local crops, dynamic cropping enabled, and at most nine
local crops.

The presets from the model card map to these values:
