...
How HiCache write/load timing interacts with pipeline parallelism and speculative decoding — and which combinations startup allows — is in [PP, HiCache, and speculative decoding compatibility](./pp_hicache_spec_compatibility).
