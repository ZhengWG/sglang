...
Pipeline parallelism is compatible with [HiCache](./hicache), but not with [speculative decoding](./speculative_decoding). For the timelines (PP microbatches, draft/verify/extend, HiCache offload) and the Kimi / DeepSeek / GLM recipe matrix, see [PP, HiCache, and speculative decoding compatibility](./pp_hicache_spec_compatibility).
