...
class CommonKVManager(BaseKVManager):
    def __init__(
        ...
        # Sender modes: PREFILL (KV) and ENCODE (Embedding)
        if self.disaggregation_mode in (DisaggregationMode.PREFILL, DisaggregationMode.ENCODE):
            self._register_to_bootstrap()
            self.transfer_infos: Dict[int, Dict[str, TransferInfo]] = {}
            self.decode_kv_args_table: Dict[str, KVArgsRegisterInfo] = {}
            self.pp_group = get_pp_group()
        # Receiver modes: DECODE (KV) and LANGUAGE (Embedding)
        elif self.disaggregation_mode in (DisaggregationMode.DECODE, DisaggregationMode.LANGUAGE):
