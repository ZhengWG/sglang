...
class Scheduler(
    ...
    def recv_requests(
        ...
        if recv_reqs:
            # Barrier only when the current batch actually contains
            # ShmPointerMMData (multimodal requests with SHM-backed features)
            # AND we are on the non-DP-attention path with tp > 1.
            #
            # Why needed: in the non-DP path there is a single broadcast_pyobj
            # on tp_cpu_group.  The source rank returns the original objects
            # immediately while other ranks are still in pickle.loads
            # (-> __setstate__ -> shm_open).  Without a barrier the source
            # can call materialize() / shm_unlink before others open the
            # segment.  recv_reqs is consistent across all ranks here (same
            # broadcast), so the check is deadlock-free.
            #
            # Why NOT needed for DP-attention: the control_reqs broadcast on
            # tp_cpu_group (step 3) is a collective that forces every rank to
            # complete the earlier attn_tp / attn_cp work_reqs deserializations
            # (steps 1-2, which call shm_open) before any rank returns from
            # step 3.  POSIX guarantees shm_unlink only removes the name;
            # already-open handles stay valid.
            if (
