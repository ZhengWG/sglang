...
class DataParallelController:
    ...
    def maybe_external_dp_rank_routing(self, req: Req):
        if req.routed_dp_rank is not None:
            requested_rank = req.routed_dp_rank
            if requested_rank < 0:
                raise ValueError(f"DP rank {requested_rank} must be non-negative.")
            if not self.workers:
                raise RuntimeError("No DP workers are available for routing.")
            target_rank = requested_rank % len(self.workers)
            worker = self.workers[target_rank]
            if target_rank not in self._active_workers or worker is None:
                raise ValueError(
                    f"DP rank {target_rank} (requested as {requested_rank}) "
                    "is not active."
                )
            logger.info(
                f"Direct routing {requested_rank} to DP rank {target_rank}"
            )
            sock_send(worker, req)
            return True
        return False
    ...
    def round_robin_scheduler(self, req: Req):
        ...
        while attempts < len(active):
            ...
            if self.status[slot]:
                logger.info(f"Choose worker {slot}")
