...
from sglang.srt.utils import get_bool_env_var, is_cpu
import torch
...
def l2normv2(x: torch.Tensor, layernorm_epsilon):
    ...
    if is_dp_attention_enabled():
        ...
        if tp_size > 1:
            local_norm = torch.pow(x_float.norm(p=2, dim=-1, keepdim=True), 2)
            weight_norm_square = get_attention_tp_group().all_reduce(local_norm)
            global_norm = torch.sqrt(weight_norm_square)
...
def apply_hyper_spherical_fusion(
    ...
    alpha_init_scaling: float,
    layernorm_epsilon: float = 1e-6
) -> Tuple[torch.Tensor, torch.Tensor]:
    _alpha = alpha * (alpha_init_value / alpha_init_scaling)
    hidden_states = l2norm(hidden_states, layernorm_epsilon) * _alpha
    residual = l2norm(residual, layernorm_epsilon) * (1 - _alpha)
...
def scale_operation(
    tensor: torch.Tensor,
    scaling_up_factor: torch.Tensor,
...
class ScaleUpNorm(torch.nn.Module):
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        loaded_weight = loaded_weight * (self.intermediate_size ** 0.5)
