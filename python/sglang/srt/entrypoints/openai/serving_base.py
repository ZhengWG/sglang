...
from sglang.srt.observability.utils import RECEIVED_TIME
from sglang.srt.managers.io_struct import EmbeddingReqInput, GenerateReqInput
from sglang.srt.observability.req_time_stats import monotonic_time
from sglang.srt.server_args import ServerArgs
from sglang.srt.observability.trace import contains_trace_headers, extract_trace_headers, is_tracing_enabled, log_tracing_disabled_warning
...
class OpenAIServingBase(ABC):
    ...
    async def handle_request(
        ...
        try:
            ...
            error_msg = f"Internal server error: {str(e)}" if error_code >= 500 \
                else f"Client error: {str(e)}"
            error_type = "InternalServerError" if error_code >= 500 else "BadRequestError"
            return self.create_error_response(
                message=error_msg,
                err_type=error_type,
