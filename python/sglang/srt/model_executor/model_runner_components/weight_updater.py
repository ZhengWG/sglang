...
class WeightUpdater:
    ...
    def update_weights_from_disk(
        ...
        with set_default_torch_dtype(self.model_config.dtype):
            if weights is None:
                try:
                    weight_iter = get_weight_iter(self.model_config)
                except Exception as e:
                    message = f"Failed to get weights iterator: {e}."
                    return False, message
            try:
                model = model_load_weights(
                    self.get_model(), weights if weights is not None else weight_iter
                )
            except Exception as e:
                message = (
                ...
                if weight_iter is not None:
                    del weight_iter
                gc.collect()
                rollback_iter = get_weight_iter(self.model_config)
                model_load_weights(self.get_model(), rollback_iter)
                return False, message
