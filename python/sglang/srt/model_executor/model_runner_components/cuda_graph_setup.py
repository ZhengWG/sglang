...
def capture_cuda_graphs(
    *, model_runner: ModelRunner, capture_decode_cuda_graph: bool = True
    ...
    # Complete deferred weight loading after eager-runner initialization has
    # overlapped with the weight iterator setup, but before any graph captures
    # model state.
    model_runner.finalize_post_load_weights()
...
def capture_decode_graph(*, model_runner: ModelRunner) -> DecodeGraphCapture:
    ...
    logger.info(
        f"[SpanLogs] Capture {capture_name} {graph_backend[model_runner.device]} begin. "
    ...
    logger.info(
        f"[SpanLogs] Capture {capture_name} {graph_backend[model_runner.device]} end. "
        f"elapsed={time.perf_counter() - tic:.2f} s, "
