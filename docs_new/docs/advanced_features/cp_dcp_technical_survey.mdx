...
```
序列 token:  t0 t1 t2 t3 t4 t5 t6 t7     (cp_size=4 → 切成 8 块 zigzag)
...
每层流水（各 rank 并行，不是按时间先后串行）:

  ① 本地只跑自己的 token → 得到 local K/V（只有两个 block）
  ② AllGather + 重排 → 每张卡 KV pool 里都有 [B0|B1|…|B7] 完整序
  ③ 本地 Q 对完整 KV 做 FA（见下）
  ④ 层末 Gather hidden（或下一层再 shard）
```

**为什么保留完整 KV（而不是像 DCP 只留 1/N）？**

1. Prefill CP 的目标是 **分摊 Q/FFN 算力**（每卡只算 `~1/cp_size` 的 token），不是省 decode KV 显存。
2. Zigzag 算法依赖「池里已是全局 token 序的 KV + 按因果可见长度截断」。
3. Prefill 结束后进入 decode：默认不再走 CP，需要每卡（在其 attn_tp head 分片上）已有完整序列 KV。

例外：`--enable-dsa-cache-layer-split` 会把 **不同 layer 的 DSA KV** 分到不同 CP rank（省显存），那是另一条优化，不是默认 zigzag 行为。

#### 因果「先后关系」如何保证？

关键误解：以为 cp0 必须先算完，cp1 才能算。实际上 **所有 CP rank 同时算**；因果靠 **可见 KV 长度** 表达，不靠 rank 间流水线顺序。

Zigzag 把本 rank 的 Q 再拆成两半（early `q_prev` / late `q_next`），分别限制 `cache_seqlens`：

```
例：cp_size=4，cp_rank=1 拥有 B1(early) 与 B6(late)
...
FA 调用两次（见 ZigzagCPStrategy.run_attention）:
  attn(q_prev, cache_seqlens=kv_len_prev)
  attn(q_next, cache_seqlens=kv_len_next)
  → cat → 本 rank 的 attention 输出
```

因果正确性来自：KV 已按全局位置排好 + `cache_seqlens` 截到合法前缀 + chunk 内 causal mask；**不需要** rank 之间按时间排队。
...
sequenceDiagram
  ...
  Note over C0,C3: 同一层，四卡并行
  C0->>C0: Q/K/V on B0+B7
  C1->>C1: Q/K/V on B1+B6
  C2->>C2: Q/K/V on B2+B5
  C3->>C3: Q/K/V on B3+B4
  C0<<->>C3: AllGather K/V → 每卡写满完整序
  C0->>C0: FA(q_prev,q_next) vs full KV
  C1->>C1: FA(q_prev,q_next) vs full KV
  C2->>C2: FA(q_prev,q_next) vs full KV
  C3->>C3: FA(q_prev,q_next) vs full KV
```
...
它 **不是 DCP**，也 **不改 KV 布局**。它解决的是 Prefill CP 打开后 decode 阶段的 **算力浪费**。

背景：

```
开 Prefill CP 后（示意 tp=8, attn_cp=8）:
  attn_tp_size = tp / (attn_cp * attn_dp) = 1
  → 每张卡上的 attention 线性层权重是「整份复制」的（相对正常 TP）

Prefill 时：每卡只算自己的序列分片 Q，需要「完整 head」→ 复制权重合理。

Decode 时：不再走 zigzag/interleave（is_cp_v2_active / dsa_use_prefill_cp 为假），
  若仍用整份复制权重 → 8 张卡做 8 次完全相同的全 head GEMM → 纯浪费。
```

`--enable-cp-decode-attn-tp` 做的事：

```
Decode forward 期间（context manager）:
  把 ColumnParallel / RowParallel 的 weight 按 attn_cp_rank 切一刀
  （等价于临时恢复「按 head 的 TP」）
  → 每卡只算 1/N head
  → RowParallel 输出走 all-reduce（linear.py 里 use_decode_attn_tp 触发）
  → forward 结束 restore 整份权重（留给下次 Prefill CP 用）
```

```
Prefill CP 开着、Decode Attn TP 关着:
  GPU0: 算全部 H 个 head   GPU1: 算全部 H 个 head  …  （重复劳动）

Decode Attn TP 开着:
  GPU0: 算 head[0, H/N)    GPU1: 算 head[H/N, 2H/N) … 再 AllReduce
  （恢复正常 TP 的分工；KV 仍是 Prefill CP 物化好的完整序）
```

白名单架构：DeepSeek-V4 / GLM DSA 等（见 `CP_DECODE_ATTN_TP_SUPPORTED_ARCHS`）。Prefill CP 活跃时强制关掉切片（那一步需要全 head）。
