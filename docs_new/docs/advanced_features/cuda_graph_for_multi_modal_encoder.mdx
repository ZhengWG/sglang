...
## Encoder-server (disaggregated) usage

The disaggregated encoder process (`python/sglang/srt/disaggregation/encode_server.py`)
loads the same model and dispatches `get_image_feature`/`get_video_feature`
to the `Qwen3VLMoeVisionModel.forward(...)` path, so simply exporting the
environment variable on the encoder host enables the ViT CUDA Graph:

```shell Command
SGLANG_VIT_ENABLE_CUDA_GRAPH=1 \
python3 -m sglang.srt.disaggregation.encode_server \
  --model Qwen/Qwen3.5-... \
  --tp 1 --host 0.0.0.0 --port 30005
```
