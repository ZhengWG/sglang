...
dnf install -y git numactl-devel #libnuma-dev
...
$PIP_CMD install -e "python[dev]" --extra-index-url https://download.pytorch.org/whl/${CU_VERSION} $PIP_INSTALL_SUFFIX --no-build-isolation
...
if [ "${CUSTOM_BUILD_SGL_KERNEL:-}" = "true" ]; then
    ...
    $PIP_CMD install python/sglang/kernels/aot/dist/sgl_kernel-${SGL_KERNEL_VERSION_FROM_KERNEL}+${SGL_KERNEL_CUDA_VERSION}-cp310-abi3-manylinux2014_x86_64.whl --force-reinstall $PIP_INSTALL_SUFFIX
else
    # example: https://artifacts.antgroup-inc.cn/simple-dev/sgl-kernel/sgl_kernel-0.3.16.post4-cp310-abi3-linux_x86_64.whl
    # try 180 times, that is to say, wait at most 30 minutes
    for i in {1..180}; do
      curl -f -OL https://artifacts.antgroup-inc.cn/simple-dev/sgl-kernel/sgl_kernel-${SGL_KERNEL_VERSION_FROM_SRT}-cp310-abi3-linux_x86_64.whl || true
      if [ -f sgl_kernel-${SGL_KERNEL_VERSION_FROM_SRT}-cp310-abi3-linux_x86_64.whl ]; then
        break
      fi
      sleep 10
    done
    if ! [ -f sgl_kernel-${SGL_KERNEL_VERSION_FROM_SRT}-cp310-abi3-linux_x86_64.whl ]; then
      echo download sgl-kernel whl timeout, now quit...
      exit -1
    fi
    $PIP_CMD install ./sgl_kernel-${SGL_KERNEL_VERSION_FROM_SRT}-cp310-abi3-linux_x86_64.whl --force-reinstall $PIP_INSTALL_SUFFIX
...
if [ "$IS_BLACKWELL" != "1" ]; then
    ...
    git clone --branch v0.3.3 --depth 1 https://gitee.com/lllcho/lmms-eval.git
