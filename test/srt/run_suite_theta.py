...
suites.update({"theta": 
  suites['per-commit'] + 
  suites['per-commit-2-gpu'] + 
  suites['per-commit-4-gpu'] + 
  suites['per-commit-8-gpu'] + 
  suites['per-commit-8-gpu-h20']
})
...
def test_theta():
    ...
    exit_code = run_unittest_files(files, 1800)
    assert exit_code == 0, 'test failed, please check test log'
